 ...
 *
 * ⚠ F95, FIXED BY TASK 6b-3: THE `RETURN` CLAUSES STAY, THE SENTENCE ABOUT
 * READING THEM CHANGED. D173(Q5) made the writing templates self-verifying and
 * the check line told the agent to read `m.id`, `produced` and `supportedBy` —
 * fields it can never see, because `write_neo4j_cypher` DISCARDS result rows and
 * answers with a write summary. Re-probed against the live graph on 2026-08-21:
 * a matched write answers `{_contains_updates: true, properties_set: 1}` and an
 * unmatched one answers `{}`. The templates keep their `RETURN` (it is correct
 * through the driver, it costs nothing through the tool, and D169 authored it);
 * only the instruction for READING the answer was rewritten, so the line count
 * below is unchanged at 19 and D147(e)'s pinned test still holds.
 */
export function memoryContractLines(ctx: MemoryContractContext): readonly string[] {
  ...
  return [
    ...
    `FINDING WHERE SOMETHING LIVES: when you need to locate a file in this repository — before running rg, ls, dir or a directory walk — run the FIND A FILE template instead; it answers from an index of every tracked file in one round trip, with no output to page through.`,
    ...
    `CHECK WHAT EACH WRITE RETURNED, AND KNOW WHAT SHAPE IT COMES BACK IN: this MCP server answers a write with a SUMMARY, not with the rows of the RETURN clause, so a success reads like {nodes_created: 1, relationships_created: 2, properties_set: 8} while a write whose MATCH found nothing reads as an EMPTY OBJECT. An empty object, or nodes_created 0, means NOTHING WAS WRITTEN because the cited path or sha is not in the graph; check the value you passed and retry. A malformed query comes back as a tool ERROR, but a write whose MATCH found nothing comes back as SUCCESS — only reading the result tells those two apart.`,
    `If a write comes back as an empty object, a MATCH found nothing — check $relPath with the READ tool rather than switching to CREATE, because a CREATE-ed :File would be a second node under a name Chorus does not index.`,
