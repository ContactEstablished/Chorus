...
describe('window controls (Task 3c-2 / D74) — the phase\'s ONE IPC exception', () => {
  it('names exactly the four channels the exception allows, and no more', () => {
    ...
    //
    // ⚠ 76 → 82 IS v17'S SIX, AND THEY ARE TWO UNRELATED GROUPS THAT HAPPEN TO
    ...
    //      must never exist.
    expect(Object.keys(IpcChannel)).toHaveLength(82)
  })
...
describe('cliDetectRequestSchema — the refresh flag (CLI staleness)', () => {
  ...
  it('⚠ adds no channel — the count still holds at 82', () => {
    ...
    // twin, which is the one place it belongs), and now to 82 together for
    // Task 6-3's five `memory:*` channels PLUS v17's context-ring pair and
    // agent-lock four. If you are here to change one number, change the other
    // in the same commit — one at 82 and one at 71 is a failed gate, not a
    // rounding error.
    //
    ...
    // ⚠ AND IT HAPPENED AGAIN AT 82 — see the twin. Twice is a pattern, not
    // bad luck: this number is a SUM over every branch in flight, and the only
    // safe way to move it is to re-count after the merge rather than to add
    // your own delta to whatever the file said before you branched.
    expect(Object.keys(IpcChannel)).toHaveLength(82)
  })
})
