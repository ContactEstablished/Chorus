...
| **3-4** | **First real Settings view (D29), built to the D38 design skeleton.** D37 chore first (F25 `layout:get` tolerance), then the renderer-only task: left settings nav + grouped provider cards with nested credential rows (`docs/design/Chorus Settings Providers.dc.html`, skeleton only — current idiom, no visual system until Phase 3c); CRUD over 3-2's channels; auth methods from 3-3's `adapter:list`. Plaintext keys are write-only: renderer→main once, never read back; **the mock's masked key hints are D33-overridden and not built.** | 3-3 | — |
...
| `src/main/services/scrubber.ts` | — | — | — | — | **create** | consume (unchanged) |
...
| `src/renderer/src/views/Settings*.vue` | — | — | — | **create** | — | **edit** (Test key on Credentials; `model` input on Providers, D48) |
...
- **Baseline — SUPERSEDED, see the current figure.** At `fb3201e` (3-1) it was 160/160 across 8 files. **Current, coordinator-verified 2026-07-24 at `ddb5454`: `npm run typecheck` exits 0 · `npx vitest run` = 254/254 across 12 files · `npm run grep:secrets` reports `clean (6 patterns over src/, scripts/, _verify/, package.json, root configs)`.** Working tree carries only the untracked `TASK-3-5-REVIEW-FABLE.md`.
...
- **Migrations are a hand-rolled numbered array.** `MIGRATIONS: string[]` in `storage.ts`, applied in order inside a transaction by `migrate()`, tracked in `schema_migrations`. **⚠ COUNT UPDATED: five migrations are applied as of 2026-07-24** (v5 landed with Task 3-2 on the real dev DB at `2026-07-23T13:04:06.301Z`), and **Task 3-6 adds v6** (`provider_configs.model`, D48). The figure below is the 2026-07-22 snapshot, kept for its v4 timestamp evidence: **coordinator-verified against the real dev DB 2026-07-22: exactly 4 applied** (v4 `applied_at` `2026-07-20T16:57:49.534Z`, untouched). Task 3-2 adds **v5** and it must apply **in place, with zero data loss**.
...
- **Verify CLI/provider flags and env-var names against the tool's own `--help`/docs at execution time (D4)** — never from training-data memory. This binds hardest in 3-6: `ANTHROPIC_API_KEY`, `ANTHROPIC_BASE_URL`, `OPENAI_API_KEY`, and Codex's config-file conventions must each be confirmed against the installed CLI before being hardcoded. **⚠ Both CLIs moved on 2026-07-24: `claude.exe` is now 2.1.218 (was 2.1.215) and `codex-cli` is 0.145.0 (was 0.144.6)**, so every capability claim in `codexAdapter` — verified against 0.144.6 — is now unverified; 0.145.0 advertises a `/skills` surface while the adapter declares `skills: false`. **D44 (2026-07-24) makes the hardest part of this moot:** Claude/Codex/Kimi are subscription-only by product policy, so no key is injected into them and the Steps 1a/1b precedence questions do not arise.
...
| G3 | **One** intentional narrated commit per execution session — **amended per-session, four times so far**: D32 (3-1), D36 (3-3), D37 (3-4), and **D46 (3-6: the scrub-seam chore, then BYOK)**. Each amendment is for that session only. |
...
- [x] **`AgentAdapter` refactor is behavior-neutral** — proven by in-session before/after diffs (`cli:detect` byte-identical on pre-existing fields; agent command lines identical; launch/attach/restart/restore/close unchanged); baseline grew 193 → 224 (3-3, `46ad9b7` + `62fc236`).
- [x] **The renderer no longer hardcodes agent names or labels in the launch flow** — `LaunchDialog`'s `labels`/`AGENT_KINDS` are gone, cards render from adapter-supplied wire data; the not-found disabled card survives. (Three pre-existing `labels[` records remain in FilmstripRenderer/TerminalPane/palette — outside 3-3's scope table; natural cleanup when those components are next touched.) (3-3, `62fc236`)
- [x] **Settings view** — providers and credential profiles created, listed, replaced, and deleted through the real UI (D38 skeleton); a stored key never rendered, never returned over IPC, unrecoverable after entry — proven by the implementer's six-way sweep with test-the-test shown red-then-green, and re-proven by the coordinator's independent sweep on the real dev DB with a different planted key (3-4, `e8d4e85` + `899df07`).
- [x] **Scrubber** — planted probe value printed into a live terminal renders `[REDACTED-CREDENTIAL]` on screen, in the ring buffer, and in `attach()`'s replay after a remount; a value split across two PTY writes 41 ms apart reassembles into ONE placeholder with no fragment ≥ 8 chars; registered and unregistered TUIs render identically. **Coordinator-verified 2026-07-24 by a full independent runtime re-drive on the REAL dev DB** — a within-boot A/B (registered: `placeholder 3 · valuePresent false · maxFragment 0`; unregistered control: `placeholder 0 · valuePresent TRUE`). Surfaced **F26** (3-5, `d3b6f30` code + `ddb5454` anchor, per **D40**).
- [ ] **Phase milestone (runtime-proven, G2)** — an agent launches with a BYOK credential injected as an env var into its child PTY; the key appears in **no** command line (`Get-CimInstance Win32_Process` on the live process tree), **no** log line, **no** ring buffer, and **no** renderer-reachable surface; a subscription-auth launch still works with no key injected and no allow-list applied (3-6). **Amended by D47:** the vehicle is the **OpenRouter route** — codex launched with `-c model_providers.*` overrides and the key injected under the name `env_key` designates, driving a non-GPT model — and the agent must **demonstrably answer a prompt** through it. The milestone closes on a real end-to-end proof, not on machinery that compiles and is never exercised. **No new agent kind is involved, so the "no new agent kinds" non-goal below still holds.**
...
- **No `SessionManager` PTY/API session split.** D34's Q2 ruling puts the boundary at both adapter and session level eventually; **Phase 3 restructures only the adapter side** — `SessionManager` stays PTY-only. **⚠ Task 3-6's Step-0 scrub-seam chore (D46) does NOT violate this and must not be mistaken for it.** Extracting the scrub/buffer/broadcast pipeline into `sessionOutput.ts` reshapes the *existing PTY path* into a form a second session type could later reuse; it adds **no** api-mode code, **no** `ApiSessionHandle` implementation, and **no** second session type. `SessionManager` still owns exactly one kind of session when 3-6 ends.
