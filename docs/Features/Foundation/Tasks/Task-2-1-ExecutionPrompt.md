...
- `src/main/db/schema.ts` — Drizzle table defs for projects/pane_layouts/settings/schema_migrations/sessions; sessions has NO worktree_id yet. ~~SQLite FOREIGN KEYS ARE NOT ENABLED~~ **— CORRECTED 2026-07-20 (F16): foreign keys ARE enforced** (better-sqlite3 12.11.1 sets `PRAGMA foreign_keys=ON` per connection), so `REFERENCES` clauses are real constraints; code checks remain for liveness/attachability.
