...
- **Existing-worktree attachability** is enforced in code: the row must exist, not be owned by a live session, and its directory must still be present. (Note the code checks are still required — SQLite FKs constrain *referential existence*, not liveness or attachability.)
- **⚠ FOREIGN KEYS ARE ENFORCED (F16 — corrected 2026-07-20).** better-sqlite3 12.11.1 sets `PRAGMA foreign_keys=ON` on every connection, so `worktrees.project_id` and `worktrees.session_id` are **real constraints**, not documentation. Consequences for this task's ordering, all of which the flow above already satisfies — do **not** reorder them:
  - `createWorktree(sessionId, …)` inserts a row carrying `session_id`, so **the sessions row MUST already exist** when it runs. The row-before-worktree ordering above is therefore mandatory, not stylistic.
  - The new-worktree failure path calls `storage.deleteSession(row.id)`. This is safe **because `createWorktree` deletes its own journal row on every failure path** (verified in `worktrees.ts` at `624f3da`/`8bab784`: the catch block always calls `deleteWorktreeRow(id)` before rethrowing or retrying). If a future edit makes `createWorktree` leave a row behind on failure, this `deleteSession` will throw `SQLITE_CONSTRAINT_FOREIGNKEY` — detach or delete the worktree row first.
- **⚠ Crash-window pointer asymmetry (F18).** `sessions.worktree_id` is written only by `activateWorktreeForSession`. If a crash lands between `git worktree add` succeeding and activation, the boot reconcile promotes the worktree row to `active` but **does not write the session-side pointer** — leaving `worktrees.session_id` set and `sessions.worktree_id` NULL. Because the branch label below resolves **from the session side**, such a pane renders with **no branch label** (and, after 2-4, no diff summary) despite living in a worktree. **Decide this at 2-2 execution** and state the choice in the commit: either (a) resolve `branch` from the `worktrees` table by `session_id` rather than via `sessions.worktree_id` (read-path fix, no reconcile change), or (b) have reconcile's promote repair the session pointer too (upholds resolution (a)'s both-pointers-move-together invariant, but makes reconcile write session rows). **(a) is the smaller change and is recommended.**
