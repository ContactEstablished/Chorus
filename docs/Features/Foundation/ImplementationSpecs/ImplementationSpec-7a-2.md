...
- **It does not touch the activity mechanism.** `shell` declares `hooks: null`, so
  `sessionManager.ts:820` registers the session **output-driven** (D182) and its pane's light is minted
  from PTY output on the 10-second `OUTPUT_STALE_MS` window. A shell echoes keystrokes, so the bar
  lights while a human types. **That is D182 working; it is not this task's to change.** The pane still
...
```ts
  ...
  // OUTPUT-DRIVEN at spawn (sessionManager.ts:820, D182), so its bar lights
...
8. **The activity light.** Type in the Terminal pane and watch the rail. It lights while output flows
   and goes out roughly ten seconds later (`OUTPUT_STALE_MS`, D182). **Record it; change nothing.**
