...
_Issued 2026-07-21 · Status: AWAITING FINDINGS · Decision owner: Matthew Wilson · Recorder: Claude (roadmap §6, to be filed as **D34**)_
...
- **Immediately before this task**, a credential vault lands (a separate council, CR-3.0): keys encrypted at rest, decrypted in main at launch, injected as **environment variables into the child PTY, never as CLI arguments**. The adapter's launch-building method is the seam where a resolved credential becomes a child process's environment.
...
4. **The credential seam.** `buildLaunch` receives a resolved credential and must place it in the child's **environment**, never in arguments — a hard rule from the parallel vault review (CR-3.0). The interface should make the wrong thing hard to write.
